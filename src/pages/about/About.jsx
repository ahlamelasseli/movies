  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-6">
      <div className="max-w-2xl mx-auto">
        ...
        <img src={movie.poster} alt={movie.title} className="w-full h-96 object-cover rounded-lg mb-4" />
        <h1 className="text-3xl font-bold">{movie.title}</h1>
        <p className="text-gray-400">{movie.subtitle}</p>
        <p className="mt-4">{movie.description}</p>
        <div className="mt-4 text-sm">
          <p><strong>Date & Time:</strong> {movie.date}</p>
          <p><strong>Screen:</strong> {movie.screen} | <strong>Row:</strong> {movie.row} | <strong>Seat:</strong> {movie.seat}</p>
          {movie.imax && <p className="text-blue-400 mt-2 font-bold">IMAX</p>}
        </div>
      </div>
    </div>
  );
